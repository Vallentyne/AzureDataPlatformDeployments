...
sudo ip rule add from 10.22.0.37 to 168.63.129.16 lookup mrz
sudo ip rule add from 10.22.0.101 to 168.63.129.16 lookup int

    #WARNING this is not permanent!! also watch out for the ETHX number
    #sudo ip route add 168.63.129.16 via 10.22.0.33 dev eth2 table mrz
    #sudo ip route add 168.63.129.16 via 10.22.0.97 dev eth1 table int

sudo route add -net 10.0.0.0/8 dev eth2 
sudo iptables -t nat -A POSTROUTING -o eth2 -j MASQUERADE
    ...
    #sudo route add 168.63.129.16 gw 10.22.0.33 metric 100
    #sudo route add 168.63.129.16 gw 10.22.0.97 metric 100

    #now a general route to MRZ
    #route add -net 10.22.4.0/22 gw 10.22.0.97 dev eth1 
    #sudo iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE
    #sudo iptables -A PREROUTING -t nat -i eth0 -p tcp --dport 8080 -j DNAT --to 10.22.4.132:80
    #sudo iptables -A FORWARD -p tcp -d 10.22.4.132 --dport 80 -j ACCEPT
